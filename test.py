...
class PacmanUI:
    def __init__(self, root, db):
        self.root = root
        self.db = db
        self.root.title("Pacman Game and Boolean Formula Builder")
    ...
    def show_database(self):
        ...
        self.tree = ttk.Treeview(self.content_frame)  # Notice the change here
        self.tree['columns'] = ('ID', 'Layout Name', 'Ghosts')

        self.tree.column("#0", width=0, stretch=tk.NO)
        self.tree.column('ID', anchor=tk.CENTER, width=80)
        self.tree.column('Layout Name', anchor=tk.W, width=120)
        ...
        for row in data:
            self.tree.insert('', tk.END, values=row)  # Assuming row matches columns
        ...
        def on_double_click(event):
            item = self.tree.selection()[0]  # Adjusted to use self.tree
    ...
    def delete_selected_level(self):
        ...
        if selected_item:
            ...
            level_id, level_name, _ = item['values']  # Assuming the level name is the second value in the 'values' list
